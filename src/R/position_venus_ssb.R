...
# Calculate the position of Venus using the de441 data

position_venus_ssb <- function(jd_tdb)
{
  df <- extract_body_data("Venus", jd_tdb)
  
  # Calculate the subinterval
  length_of_subinterval <- DAYSPERBLOCK / NUMSUBINTVENUS
  ...
  venus_coeff_x <- rep(0.0, NUMCOEFFVENUS)
  venus_coeff_y <- rep(0.0, NUMCOEFFVENUS)
  venus_coeff_z <- rep(0.0, NUMCOEFFVENUS)
  
  for (i in seq(from = NUMCOEFFVENUS, to = 1, by = -1)){
    venus_coeff_x[i] <- temp[i+3]
    venus_coeff_y[i] <- temp[i+3+NUMCOEFFVENUS]
    venus_coeff_z[i] <- temp[i+3+2*NUMCOEFFVENUS]
  }
  
  # Convert from lists to numeric vectors. For some unknown reason, the previous
  # loop converts the numeric vectors to lists
  venus_coeff_x <- as.numeric(unlist(venus_coeff_x))
  venus_coeff_y <- as.numeric(unlist(venus_coeff_y))
  venus_coeff_z <- as.numeric(unlist(venus_coeff_z))
  ...
  chebyshev_pos <- rep(0.0, NUMCOEFFVENUS)
  chebyshev_pos[1] <- 1.0
  chebyshev_pos[2] <- x
  
  chebyshev_vel <- rep(0.0, NUMCOEFFVENUS)
  chebyshev_vel[1] <- 0.0
  chebyshev_vel[2] <- 1.0
  
  # Calculate the position polynomial
  for (i in seq(from = 3, to = NUMCOEFFVENUS, by = 1)){
  ...
  v <- 0
  for (v in seq(from = NUMCOEFFVENUS, to = 1, by = -1)){
    pos_vel[1,1] <- pos_vel[1,1] + (chebyshev_pos[v] * venus_coeff_x[v])
    pos_vel[1,2] <- pos_vel[1,2] + (chebyshev_pos[v] * venus_coeff_y[v])
    pos_vel[1,3] <- pos_vel[1,3] + (chebyshev_pos[v] * venus_coeff_z[v])
    
    pos_vel[2,1] <- pos_vel[2,1] + (chebyshev_vel[v] * venus_coeff_x[v])
    pos_vel[2,2] <- pos_vel[2,2] + (chebyshev_vel[v] * venus_coeff_y[v])
    pos_vel[2,3] <- pos_vel[2,3] + (chebyshev_vel[v] * venus_coeff_z[v])
  }
  
  # Scale the velocity
  scale_value <- 2 * NUMSUBINTVENUS / DAYSPERBLOCK
