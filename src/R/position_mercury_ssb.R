...
# Calculate the position of Mercury using the de441 data

position_mercury_ssb <- function(jd_tdb)
{
  df <- extract_body_data("Mercury", jd_tdb)
  
  # Calculate the subinterval
  length_of_subinterval <- DAYSPERBLOCK / NUMSUBINTMERCURY
  ...
  mercury_coeff_x <- rep(0.0, NUMCOEFFMERCURY)
  mercury_coeff_y <- rep(0.0, NUMCOEFFMERCURY)
  mercury_coeff_z <- rep(0.0, NUMCOEFFMERCURY)
  
  for (i in seq(from = NUMCOEFFMERCURY, to = 1, by = -1)){
    mercury_coeff_x[i] <- temp[i+3]
    mercury_coeff_y[i] <- temp[i+3+NUMCOEFFMERCURY]
    mercury_coeff_z[i] <- temp[i+3+2*NUMCOEFFMERCURY]
  }
  
  # Convert from lists to numeric vectors. For some unknown reason, the previous
  # loop converts the numeric vectors to lists
  mercury_coeff_x <- as.numeric(unlist(mercury_coeff_x))
  mercury_coeff_y <- as.numeric(unlist(mercury_coeff_y))
  mercury_coeff_z <- as.numeric(unlist(mercury_coeff_z))
  ...
  chebyshev_pos <- rep(0.0, NUMCOEFFMERCURY)
  chebyshev_pos[1] <- 1.0
  chebyshev_pos[2] <- x
  
  chebyshev_vel <- rep(0.0, NUMCOEFFMERCURY)
  chebyshev_vel[1] <- 0.0
  chebyshev_vel[2] <- 1.0
  
  # Calculate the position polynomial
  for (i in seq(from = 3, to = NUMCOEFFMERCURY, by = 1)){
  ...
  v <- 0
  for (v in seq(from = NUMCOEFFMERCURY, to = 1, by = -1)){
    pos_vel[1,1] <- pos_vel[1,1] + (chebyshev_pos[v] * mercury_coeff_x[v])
    pos_vel[1,2] <- pos_vel[1,2] + (chebyshev_pos[v] * mercury_coeff_y[v])
    pos_vel[1,3] <- pos_vel[1,3] + (chebyshev_pos[v] * mercury_coeff_z[v])
    
    pos_vel[2,1] <- pos_vel[2,1] + (chebyshev_vel[v] * mercury_coeff_x[v])
    pos_vel[2,2] <- pos_vel[2,2] + (chebyshev_vel[v] * mercury_coeff_y[v])
    pos_vel[2,3] <- pos_vel[2,3] + (chebyshev_vel[v] * mercury_coeff_z[v])
  }
  
  # Scale the velocity
  scale_value <- 2 * NUMSUBINTMERCURY / DAYSPERBLOCK
