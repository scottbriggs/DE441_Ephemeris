...
# Calculate the position of Mars using the de441 data

position_mars_ssb <- function(jd_tdb)
{
  df <- extract_body_data("Mars", jd_tdb)
  
  # Calculate the subinterval
  length_of_subinterval <- DAYSPERBLOCK / NUMSUBINTMARS
  ...
  mars_coeff_x <- rep(0.0, NUMCOEFFMARS)
  mars_coeff_y <- rep(0.0, NUMCOEFFMARS)
  mars_coeff_z <- rep(0.0, NUMCOEFFMARS)
  
  for (i in seq(from = NUMCOEFFMARS, to = 1, by = -1)){
    mars_coeff_x[i] <- temp[i+3]
    mars_coeff_y[i] <- temp[i+3+NUMCOEFFMARS]
    mars_coeff_z[i] <- temp[i+3+2*NUMCOEFFMARS]
  }
  
  # Convert from lists to numeric vectors. For some unknown reason, the previous
  # loop converts the numeric vectors to lists
  mars_coeff_x <- as.numeric(unlist(mars_coeff_x))
  mars_coeff_y <- as.numeric(unlist(mars_coeff_y))
  mars_coeff_z <- as.numeric(unlist(mars_coeff_z))
  ...
  chebyshev_pos <- rep(0.0, NUMCOEFFMARS)
  chebyshev_pos[1] <- 1.0
  chebyshev_pos[2] <- x
  
  chebyshev_vel <- rep(0.0, NUMCOEFFMARS)
  chebyshev_vel[1] <- 0.0
  chebyshev_vel[2] <- 1.0
  
  # Calculate the position polynomial
  for (i in seq(from = 3, to = NUMCOEFFMARS, by = 1)){
  ...
  v <- 0
  for (v in seq(from = NUMCOEFFMARS, to = 1, by = -1)){
    pos_vel[1,1] <- pos_vel[1,1] + (chebyshev_pos[v] * mars_coeff_x[v])
    pos_vel[1,2] <- pos_vel[1,2] + (chebyshev_pos[v] * mars_coeff_y[v])
    pos_vel[1,3] <- pos_vel[1,3] + (chebyshev_pos[v] * mars_coeff_z[v])
    
    pos_vel[2,1] <- pos_vel[2,1] + (chebyshev_vel[v] * mars_coeff_x[v])
    pos_vel[2,2] <- pos_vel[2,2] + (chebyshev_vel[v] * mars_coeff_y[v])
    pos_vel[2,3] <- pos_vel[2,3] + (chebyshev_vel[v] * mars_coeff_z[v])
  }
  
  # Scale the velocity
  scale_value <- 2 * NUMSUBINTMARS / DAYSPERBLOCK
