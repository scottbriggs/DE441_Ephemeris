...
# Calculate the position of Jupiter using the de441 data

position_jupiter_ssb <- function(jd_tdb)
{
  df <- extract_body_data("Jupiter", jd_tdb)
  
  # Calculate the subinterval
  length_of_subinterval <- DAYSPERBLOCK / NUMSUBINTJUPITER
  ...
  jupiter_coeff_x <- rep(0.0, NUMCOEFFJUPITER)
  jupiter_coeff_y <- rep(0.0, NUMCOEFFJUPITER)
  jupiter_coeff_z <- rep(0.0, NUMCOEFFJUPITER)
  
  for (i in seq(from = NUMCOEFFJUPITER, to = 1, by = -1)){
    jupiter_coeff_x[i] <- temp[i+3]
    jupiter_coeff_y[i] <- temp[i+3+NUMCOEFFJUPITER]
    jupiter_coeff_z[i] <- temp[i+3+2*NUMCOEFFJUPITER]
  }
  
  # Convert from lists to numeric vectors. For some unknown reason, the previous
  # loop converts the numeric vectors to lists
  jupiter_coeff_x <- as.numeric(unlist(jupiter_coeff_x))
  jupiter_coeff_y <- as.numeric(unlist(jupiter_coeff_y))
  jupiter_coeff_z <- as.numeric(unlist(jupiter_coeff_z))
  ...
  chebyshev_pos <- rep(0.0, NUMCOEFFJUPITER)
  chebyshev_pos[1] <- 1.0
  chebyshev_pos[2] <- x
  
  chebyshev_vel <- rep(0.0, NUMCOEFFJUPITER)
  chebyshev_vel[1] <- 0.0
  chebyshev_vel[2] <- 1.0
  
  # Calculate the position polynomial
  for (i in seq(from = 3, to = NUMCOEFFJUPITER, by = 1)){
  ...
  v <- 0
  for (v in seq(from = NUMCOEFFJUPITER, to = 1, by = -1)){
    pos_vel[1,1] <- pos_vel[1,1] + (chebyshev_pos[v] * jupiter_coeff_x[v])
    pos_vel[1,2] <- pos_vel[1,2] + (chebyshev_pos[v] * jupiter_coeff_y[v])
    pos_vel[1,3] <- pos_vel[1,3] + (chebyshev_pos[v] * jupiter_coeff_z[v])
    
    pos_vel[2,1] <- pos_vel[2,1] + (chebyshev_vel[v] * jupiter_coeff_x[v])
    pos_vel[2,2] <- pos_vel[2,2] + (chebyshev_vel[v] * jupiter_coeff_y[v])
    pos_vel[2,3] <- pos_vel[2,3] + (chebyshev_vel[v] * jupiter_coeff_z[v])
  }
  
  # Scale the velocity
  scale_value <- 2 * NUMSUBINTJUPITER / DAYSPERBLOCK
